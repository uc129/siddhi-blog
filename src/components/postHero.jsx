...
const PostHero = ({ date, author, volume, postNumber, title, slug, bgImage }) => {
    ...
    const fetchImage = async (id) => {
        ...
        await axiosClient.get(`/blog/images/id/${id}`)
            .then((res) => { data = res.data; setImage(data.url); console.log('data.url', data.url);; if (data.url) { setImageFound(true); setIsLoading(false) } })
            .catch((err) => { console.log(err, axiosClient.defaults.baseURL) })
        return data
    }
